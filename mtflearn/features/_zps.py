...
class ZPs(BaseEstimator, TransformerMixin):
    ...
    def _transform_fft_convolve(self, image: np.ndarray) -> zmoments:
        ...
        # FFT convolution vs direct integration correction:
        # - Direct integration: ∫∫ f(x,y) * V_nm(x,y) dx dy (what we want)
        # - FFT convolution:    ∫∫ f(x,y) * V_nm(-x,-y) dx dy (what we compute)
        # - Zernike symmetry:   V_nm(-x,-y) = (-1)^n * V_nm(x,y)
        # Therefore: conv_result = (-1)^n * desired_result, so multiply by (-1)^n to correct
        f = 1 - self.n % 2  # (-1)^n = 1 for even n, -1 for odd n
        f[f == 0] = -1
