"""
The follow code is directly adapted from https://github.com/ercius/openNCEM/blob/main/ncempy/io/dm.py
...
class fileDM:
    ...
    def getMetadata(self, index):
        ...
        good_keys = ['Calibrations', 'Acquisition', 'DataBar', 'EELS', 'Meta Data', 'Microscope Info',
                     '4Dcamera Parameters', 'Session Info']
...
def dmReader(filename, dSetNum=0, verbose=False, on_memory=True):
    ...
    for pixel_size, pixel_origin, pixel_unit, sh in zip(im1['pixelSize'], im1['pixelOrigin'], im1['pixelUnit'],
        ...
        if sh == 1:
            ...
            pixel_origin = round(-1 * pixel_origin * pixel_size,
                                 ndigits=4)  # Multiply by pixelSize to get the correct origin
