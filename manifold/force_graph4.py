...
def apply_attraction_force(node1, node2, alpha, lr, weight):
    x_dist = node1.x - node2.x
    y_dist = node1.y - node2.y
    distance = np.hypot(x_dist, y_dist)
    force = 1 / (distance ** (alpha) + 1)
...
@numba.njit(fastmath=True)
def apply_repulsion_force(node1, node2, alpha, lr, weight):
    x_dist = node1.x - node2.x
    y_dist = node1.y - node2.y
    distance = np.hypot(x_dist, y_dist)
    force = 1 / (distance ** alpha + 1)
...
@numba.njit(fastmath=True)
def apply_attraction_force_stage2(node1, node2, alpha, lr, weight):
    a, b = (1.5769434603935901, 0.8950608781603765)
    x_dist = node1.x - node2.x
    y_dist = node1.y - node2.y
    distance = np.hypot(x_dist, y_dist)
    # force = 2 * a * b * np.power(distance, 2 * b - 2) / (np.power(distance, 2 * b) + 1)
    force = alpha / (distance ** 2 + 1)
...
@numba.njit(fastmath=True)
def apply_repulsion_force_stage2(node1, node2, alpha, lr, weight):
    a, b = (1.5769434603935901, 0.8950608781603765)
    x_dist = node1.x - node2.x
    y_dist = node1.y - node2.y
    distance = np.hypot(x_dist, y_dist)
    # force = 2*b / (distance ** 2+1) / (a * np.power(distance, 2 * b) + 1)
    force = alpha / (distance ** 4 + 1)
...
@numba.njit(fastmath=True)
def optimize_stage1(num_iterations, nodes, pairs, alphas, num_negative_samples, nbrs_ind, learning_rate,
                    logs):
    num_nodes = len(nodes)
    lr = learning_rate
    alpha1 = alphas[0]
    ...
    for n in range(0, num_iterations):
        for pair in pairs:
            ...
            apply_attraction_force(node1, node2, alpha1, lr, weight)
            ...
            for i in range(num_negative_samples):
                ...
                if do_repulsion:
                    ...
                    apply_repulsion_force(node1, negative_node, alpha2, lr, weight)
...
# Optimize layout using three settings
@numba.njit(fastmath=True)
def optimize_stage2(num_iterations, nodes, pairs, alphas, num_negative_samples, nbrs_ind, learning_rate,
                    logs):
    num_nodes = len(nodes)
    lr = learning_rate
    alpha1 = alphas[0]
    ...
    for n in range(0, num_iterations):
        for pair in pairs:
            ...
            node1 = nodes[pair.node1]
            node2 = nodes[pair.node2]
            apply_attraction_force_stage2(node1, node2, alpha1, lr, weight)
            valid_negative_ind = nbrs_ind[pair.node1]
            for i in range(num_negative_samples):
                rand_ind = np.random.randint(low=0, high=num_nodes)
                do_repulsion = True
                ...
                if do_repulsion:
                    negative_node = nodes[rand_ind]
                    apply_repulsion_force_stage2(node1, negative_node, alpha2, lr, weight)
        lr = learning_rate * (1.0 - (float(n) / float(num_iterations)))
...
def optimize_layout(num_iterations, nodes, pairs, num_negative_samples, nbrs_ind, learning_rate, settings, divide):
    ...
    logs = optimize_stage1(n_iter, nodes, pairs, alphas, num_negative_samples, nbrs_ind, lr, logs)
    alphas = settings[2: 4]
    n_iter = int(num_iterations * (1 - divide))
    logs = optimize_stage2(n_iter, nodes, pairs, alphas, num_negative_samples, nbrs_ind, lr, logs)
    return logs
