...
class HexCells:
    ...
    def __init__(self, pts, dx=0, dy=0):
        ...
        self.ij = ij + [vmax // 2, vmax // 2]

        # get transform
        self.tform = estimate_transform('affine', self.ij, self.centers)
        self.grids = self.tform.inverse(self.pts)
    ...
    def image(self):
        ...
        vmax = max(np.ptp(self.ij[:, 0]), np.ptp(self.ij[:, 1]))
        cells_matrix = np.ones(shape=(vmax + 1, vmax + 1)) * (-1)
        for idx, (i, j) in enumerate(self.ij):
    ...
    def connected_cells(self):
        ...
        areas = areas[ind]
        components = np.array([components[e] for e in ind], dtype=object)
        # unique areas and components
        components_, lbs = unique_components(components)
        areas_ = np.array([areas[np.where(lbs == e)[0][0]] for e in np.unique(lbs)])
        components_ = [ConnectedCells(data=e, tform=self.tform) for e in components_]
        return components_

    @property
    def unique_cells(self):
        lbs, inds = np.unique(self.cell_lbs, return_index=True)
        for i in inds:
            poly = self.grids[self.regions[i]]
        return None
    ...
    def set_cell_lbs(self, pos, lbs):
        ...
        self.cell_lbs = reorder_lbs(self.cell_lbs)
...
class Cell:
    ...
    def __init__(self, pos, lbs, center=None):
        self.pos = pos
        self.lbs = lbs
...
class ConnectedCells:
    ...
    def __init__(self, data, tform, colors=None):
        self.mask = data > 0
        ...
        pts, centers= cells2points(self.data, return_centers=True)
        self.pairs, self.inds = get_pairs(pts, centers)
        self.pts = tform(pts)
        self.centers = tform(centers)
        p0 = self.pts.mean(axis=0)
        self.pts = self.pts - p0
        self.centers = self.centers - p0

    @property
    def cells(self):
        return self.pts[self.inds]
