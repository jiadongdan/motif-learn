...
class HexNet:
    ...
    def __init__(self, pts, ref=None, a=None, b=None, i=1, j=2):
        self.pts_all = pts
        self.ind = get_hexagonal_ind(self.pts_all)
        self.pts = self.pts_all[self.ind[:, 0]]

        if ref is None:
            self.ref = get_reference_hexagon(self.pts_all)
    ...
    def get_strain(self):
        ...
        return exx, eyy, 0.5*(exy+eyx)

    # this generate very similar results above
    def get_strain_F(self):
        pts1 = self.pts + self.a
        pts2 = self.pts + self.b
        ...
        u = self.pts_all[ind1] - self.pts
        v = self.pts_all[ind2] - self.pts
        ...
        Fxx, Fxy = self.matrix.dot(uvx)
        Fyx, Fyy = self.matrix.dot(uvy)

        # Green-Lagrangian strain tensor
        # https://www.ovito.org/docs/current/reference/pipelines/modifiers/elastic_strain.html
        exx = 0.5*(Fxx**2+Fxy**2 - 1)
        eyy = 0.5*(Fyy**2+Fyx**2 - 1)
        exy = 0.5*(Fxx*Fyx+Fyy*Fxy - 0)
