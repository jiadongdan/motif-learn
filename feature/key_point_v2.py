    ...
    def extract_patches(self, size, flat=False, max_patches=None, random_state=None):
        ...
        if flat == True:
            self.patches = np.array(
                [self.data[y - size:y + size + 1, x - size:x + size + 1].flatten() for (x, y) in pts])

        else:
            self.patches = np.array([self.data[y - size:y + size + 1, x - size:x + size + 1] for (x, y) in pts])
        return self.patches

    def optimize(self, r=3, subpix=False, size=None):
        ...
        if subpix:
            ...
            self.pts = (self.pts).astype(float) + np.asarray(pts) - np.array([size, size]).astype(float)
