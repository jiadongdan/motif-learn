...
class zmarray(np.ndarray):
    ...
    def maps(self, n_fold):
        a = np.zeros(len(self.m))
        for n in np.arange(1, n_fold):
            a += np.cos(np.abs(self.m) * 2 * np.pi / n_fold * n)
        a = a / (n_fold - 1)
        t = self.copy().T
        t[self.m == 0, :] = 0
        nn = norm(t, axis=0)
    ...
    def maps_(self, n_fold, normalize=True):
        states = [i for i in range(1, (self.m).max()) if i % n_fold == 0]
        ind = _get_m_states(self.m, states)
        t = self.copy()
        t[:, self.m == 0] = 0
        if normalize:
