...
class zmarray(np.ndarray):
    ...
    def rot_map(self, n_fold, mode='min'):
        # make a copy of Zernike moments
        t = self.copy()
        # remove m = 0 terms and normalize
        nn = t.select(-1).normalize()
        ms = nn.complex().m
        coeffs = (nn.complex().abs())**2

        thetas = np.array([2*np.pi/n_fold*i for i in range(n_fold)])
        # only related to cos(mt)
        cosmt = np.array([np.cos(m * t) for m in ms for t in thetas]).reshape(len(ms), -1)
        tt = coeffs.dot(cosmt)
        if mode == 'min':
            symm_map = tt.min(axis=1)
        elif mode == 'mean':
            symm_map = tt.mean(axis=1)
    ...
    def reflect_map(self, thetas=None):
