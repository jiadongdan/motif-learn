...
def optimize_layout(
    ...
    num_samples, dim = head_embedding.shape
    move_other = head_embedding.shape[0] == tail_embedding.shape[0]
    alpha = initial_alpha

    negative_sampling_step = sampling_step/negative_sample_rate
    sample_epoch_progress = sampling_step.copy()
    negative_sample_epoch_progress = negative_sampling_step.copy()
...
class UMAP(BaseEstimator):
    ...
    def _validate_parameters(self):
        ...
        if self.n_epochs is not None and (
            self.n_epochs < 1 or not isinstance(self.n_epochs, int)
        ):
    ...
    def fit(self, X, y=None):
        ...
        print(self._metric_kwds, self.random_state)
        ...
        self.embedding_, self.kkk = simplicial_set_embedding(
            self._raw_data,
            self.graph_,
            self.n_components,
            self._initial_alpha,
